...
def plot_one_box(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=3):
...
#         plot_skeleton_kpts(im, kpts, steps, orig_shape=orig_shape)
        plot_kpts(im, kpts, steps, color=color)
...
def plot_kpts(im, kpts, steps, color=None, orig_shape=None):
...
#             cv2.circle(im, (int(x_coord), int(y_coord)), radius, (int(r), int(g), int(b)), -1)
            cv2.circle(im, (int(x_coord), int(y_coord)), radius, color, -1)



def plot_images(images, targets, paths=None, fname='images.jpg', kpt_label=True, nkpt=2, steps=3, names=None, max_size=640, max_subplots=16):
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            labels = image_targets.shape[1] == 6+nkpt*steps if kpt_label else image_targets.shape[1] == 6
            ...
            if kpt_label:
                if conf is None:
                    ...
                    kpts = image_targets[:, -1*nkpt*steps:].T  # kpts for prediction
            else:
                kpts = None
            ...
            for j, box in enumerate(boxes.T):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    if kpt_label:
                        plot_one_box(box, mosaic, label=label, color=color, line_thickness=tl, kpt_label=kpt_label, kpts=kpts[:, j], steps=steps)
                    else:
