...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False, kpt_label=True):
        super(ComputeLoss, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
        self.kpt_label = kpt_label
        self.num_kpts = 2
        self.size_kpts = self.num_kpts * 3
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        if self.kpt_label:
            lkpt, lkptv = torch.zeros(1, device=device), torch.zeros(1, device=device)
            tcls, tbox, indices, anchors, tkpts = self.build_targets(p, targets)  # targets
        else:
            tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets

        sigmas = torch.tensor([0.5, 0.5], device=device) / 1.0  # 根据关键点个数进行设置
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.kpt_label:
                    ...
                    # mask
                    kpt_mask = tkpt[:, 2::3]
                    lkptv += self.BCEcls(pkpt_score, kpt_mask)

                    # oks based loss
                    if kpt_mask.sum() == 0:
                        lkpt += 0
                    else:
                        d = (pkpt_x - tkpt[:, 0::3]) ** 2 + (pkpt_y - tkpt[:, 1::3]) ** 2
    ...
    def build_targets(self, p, targets):
        ...
        if self.kpt_label:
            gain = torch.ones(7 + self.size_kpts, device=targets.device).long()  # normalized to gridspace gain
        else:
        ...
        for i in range(self.nl):
            ...
            if self.kpt_label:
                for j in range(self.num_kpts):
                    gain[6+j*3:6+j*3+2] = torch.tensor(p[i].shape)[[3, 2]]  # xyxy gain
            ...
            if self.kpt_label:
                for kpt in range(self.num_kpts):
                    kpt_mask = t[:, 6 + 3 * kpt+2: 6 + 3 * kpt + 3].repeat(1, self.num_kpts) != 0
                    t[:, 6 + 3 * kpt: 6 + 3 * kpt + 2][kpt_mask] -= gij[kpt_mask]
                tkpt.append(t[:, 6:-1])
...
class ComputeLossOTA:
    ...
    def __init__(self, model, autobalance=False, kpt_label=True):
        ...
        if kpt_label:
            self.num_kpts = 2
            self.size_kpts = 3 * self.num_kpts
        # Define criteria
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device))
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device))
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        if show:
            ...
            for i, img in enumerate(imgs):
                ...
                target = target[:, -6:].cpu().numpy()
                for j in range(target.shape[0]):
                    points = target[j]
                    point_x1 = int(points[0] * s_w)
                    point_y1 = int(points[1] * s_h)
                    point_m1 = points[2]
                    if point_m1 > 0:
                        cv2.circle(img, (point_x1, point_y1), 2, (0, 0, 255), -1)
                    point_x2 = int(points[3] * s_w)
                    point_y2 = int(points[4] * s_h)
                    point_m2 = points[5]
                    if point_m2 > 0:
                        cv2.circle(img, (point_x2, point_y2), 2, (0, 255, 0), -1)

                cv2.imwrite(root + str(self.num_call)+ '_' +str(i) + '.jpg', img)
        '''
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        lkptv, lkpt = torch.zeros(1, device=device), torch.zeros(1, device=device)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.kpt_label:
                    selected_tkpt = targets[i][:, 6:]
                    selected_tkpt[:, 0::3] *= pi.shape[3]  #x
                    selected_tkpt[:, 1::3] *= pi.shape[2]  #y

                    selected_tkpt[:, 0::3] -= gi.reshape(-1, 1).repeat(1,self.num_kpts)
                    selected_tkpt[:, 1::3] -= gj.reshape(-1, 1).repeat(1,self.num_kpts)

                    #selected_tkpt[:, 0::3] /= anchors[i][:, 0:1].repeat(1,self.num_kpts)
                    #selected_tkpt[:, 1::3] /= anchors[i][:, 1:2].repeat(1, self.num_kpts)

                    # Direct kpt prediction
                    pkpt_x = ps[:, 5+self.nc::3] * 2. - 0.5
                    pkpt_y = ps[:, 6+self.nc::3] * 2. - 0.5
                    #pkpt_x = ps[:, 5 + self.nc::3].sigmoid() * 4. - 2
                    #pkpt_y = ps[:, 6+self.nc::3].sigmoid() * 4. - 2
                    pkpt_score = ps[:, 7+self.nc::3]
                    # mask
                    kpt_mask = selected_tkpt[:, 2::3]
                    lkptv += self.BCEcls(pkpt_score, kpt_mask)
                    ...
                    if splitting:
                        #需要计算的类别和权重可以自己定义
                        ClassCalculatedList = [i for i in range(self.nc)]
                        ClassCalculatedWeights = [1.0 / self.nc] * self.nc
                        for k,cls in enumerate(ClassCalculatedList):
                            cls_mask = (selected_tcls[i] == cls)
                            if cls_mask.sum() == 0:
                                lkpt += 0
                            else:
                                cls_s = s[cls_mask]
                                cls_d = d[cls_mask]
                                cls_kpt_mask = kpt_mask[cls_mask]
                                kpt_loss_factor = (torch.sum(cls_kpt_mask != 0) + torch.sum(cls_kpt_mask == 0)) / torch.sum(
                                    cls_kpt_mask != 0)
                                cls_pkt_loss = kpt_loss_factor * ((1 - torch.exp(
                                    -cls_d / (cls_s * (4 * sigmas ** 2) + 1e-9))) * cls_kpt_mask).mean()
                                lkpt += (ClassCalculatedWeights[k] * cls_pkt_loss)
                    else:
                        # oks based loss (所有类别的关键点一起计算)
                        if kpt_mask.sum() == 0:
                            lkpt += 0
                        else:
                            d = (pkpt_x - selected_tkpt[:, 0::3]) ** 2 + (pkpt_y - selected_tkpt[:, 1::3]) ** 2
                            s = torch.prod(selected_tbox[:, -2:], dim=1, keepdim=True)
                            kpt_loss_factor = (torch.sum(kpt_mask != 0) + torch.sum(kpt_mask == 0)) / torch.sum(kpt_mask != 0)
                            lkpt += kpt_loss_factor * ((1 - torch.exp(-d / (s*(4 * sigmas ** 2) + 1e-9 ))) * kpt_mask).mean()
                            #lkpt += (d * kpt_mask).sum() / torch.sum(kpt_mask != 0)
                # Objectness
                tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * iou.detach().clamp(0).type(tobj.dtype)  # iou ratio
