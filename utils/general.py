...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None, kpt_label=False, step=3):
    ...
    if not kpt_label:
        ...
        coords[:, :4] /= gain
        clip_coords(coords[:,:4], img0_shape, False)
    else:
        coords[:, 0::step] -= pad[0]  # x padding
        coords[:, 1::step] -= pad[1]  # y padding
        coords[:, 0::step] /= gain
        coords[:, 1::step] /= gain
        clip_coords(coords, img0_shape, kpt_label=kpt_label,step=step)

    return coords


def clip_coords(boxes, img_shape, kpt_label=False, step=3):
    ...
    if kpt_label:
        boxes[:, 4::step].clamp_(0, img_shape[1])  # x1
        boxes[:, 5::step].clamp_(0, img_shape[0])  # y1
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), kpt_label=True, nc=3, nkpt=2):
    ...
    """
    if nc is None:
        nc = prediction.shape[2] - 5  if not kpt_label else prediction.shape[2] - (nkpt*3+5)
...
def non_max_suppression_export(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        kpt_label=True, nkpt=2, nc=None, labels=()):
    """Runs Non-Maximum Suppression (NMS) on inference results
